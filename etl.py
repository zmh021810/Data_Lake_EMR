...
def process_log_data(spark, input_data, output_data):
    ...
    songplays_table = df.join(song_temp_table, (df.artist == song_temp_table.artist_name)&(df.song==song_temp_table.title)&(df.length==song_temp_table.duration)).distinct().select(col("year"), col("month"), col("userId"), col("level"), col("sessionId"), col("location"), col("userAgent"), col("song_id"), col("artist_id"), col("begintime")).withColumn("song_play_index", monotonically_increasing_id())
